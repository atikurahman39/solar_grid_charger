...
# =====================================================
#  EVENT LOG  (activity log + notifications)
# =====================================================

def create_events_table():
    """Create the events table used by the dashboard activity log and the
    notification bell. Safe to call on every startup."""
    conn = get_connection()
    try:
        conn.execute("""
        CREATE TABLE IF NOT EXISTS events(
            ...
            etype     TEXT,          -- short machine tag e.g. 'anomaly','command','soc_low'
            message   TEXT,
...
def log_event(level, etype, message):
    """Insert one event. Returns the new row id (or None on failure).
    Kept defensive: logging must never crash the caller."""
    try:
        conn = get_connection()
        try:
            ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            cur = conn.execute(
                "INSERT INTO events (timestamp, level, etype, message, seen) VALUES (?,?,?,?,0)",
...
def get_events(limit=50):
    """Most recent events first (for the activity log)."""
    conn = get_connection()
    try:
        cur = conn.execute(
...
def get_unseen_count():
    """How many warning/critical events are still unseen (for the bell badge)."""
    conn = get_connection()
    try:
        cur = conn.execute(
            "SELECT COUNT(*) AS n FROM events WHERE seen = 0 AND level IN ('warning','critical')"
        )
        row = cur.fetchone()
...
def mark_events_seen():
    """Mark all events as seen (called when the user opens the bell)."""
    try:
        conn = get_connection()
        try:
            conn.execute("UPDATE events SET seen = 1 WHERE seen = 0")
            conn.commit()
        finally:
            conn.close()
    except Exception as e:
        print(f"[EVENT SEEN ERROR] {e}")
